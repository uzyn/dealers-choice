...
pub trait Evaluation {
    ...
    // Insert custom evaluation validity check here, e.g. ensuring that there are 2 cards per player and 5 community cards for hold'em.
    fn is_valid(_players: &[Hand], _board: Option<&Hand>) -> Result<(), EvaluationError> {
        Ok(())
    }
...
mod tests {
    ...
    impl Evaluation for MockEvaluation {
    ...
    fn test_compare_hands() {
        ...
        assert_eq!(
            MockEvaluation::compare_hands(&hand2, &hand3, None),
            std::cmp::Ordering::Equal
        );
        assert_eq!(
            MockEvaluation::compare_hands(&hand2, &hand2, None),
            std::cmp::Ordering::Equal
        ); // same hand
        assert_eq!(
            MockEvaluation::compare_hands(&hand1, &hand2, None),
            std::cmp::Ordering::Greater
        );
        assert_eq!(
            MockEvaluation::compare_hands(&hand2, &hand1, None),
    ...
    fn test_determine_payouts() {
        ...
        let payouts = MockEvaluation::determine_payouts(&players, None);
